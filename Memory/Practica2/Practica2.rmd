...
La evaluación de la partición test se realizará aplicando este mismo procedimiento usando la partición aprendida en el entrenamiento. En este caso, la tasa de clasificación se obtendrá buscando el dato más cercano en la partición de entrenamiento, mientras que evidentemente la tasa de reducción coincidirá con la obtenida durante el aprendizaje.
...
def SimulatedAnnealing(particion)
    ...
    while not condicionesParada()
        ...
        while num_neighbours < max_neighbours and num_success < max_success 
            ...
            # o si es peor según la probabilidad siguiente
            if diff != 0 and (diff < 0 or 
                                AleatorioUniforme(0,1) <= exp(-diff/temp))
...
def ILS(particion)
    ...
    while not condicionesParada()
        s2 = mutacionBrusca(s)
        busquedaLocal(s2,particion)
        newfit = f_objetivo(particion,s2)
...
\begin{figure}[H]
\centering
\includegraphics[width=15 cm]{./images/SA.png}
\caption{Resultados de la ejecución del algoritmo Enfriamiento Simulado.\label{fig:AGGCA}}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=15 cm]{./images/ILS.png}
\caption{Resultados de la ejecución del algoritmo Búsqueda Local Reiterada.\label{fig:AGEBLX}}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=15 cm]{./images/DE-RAND.png}
\caption{Resultados de la ejecución del algoritmo DE-RAND.\label{fig:AGECA}}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=15 cm]{./images/DE-CURRENTTOBEST.png}
\caption{Resultados de la ejecución del algoritmo DE-CURRENTTOBEST.\label{fig:AM101}}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=15 cm]{./images/global.png}
